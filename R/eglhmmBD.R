eglhmmBD <- function(data,par0,K,response,tolerance,digits,verbose,itmax,crit) {
#
# Function eglhmmBD.  To conduct the fitting of a Hidden Markov
# model when the observations are bivariate and NOT assumed to be
...
# Calculate the parameters.
    ...
    data$weights <- waits
    old.Rho <- Rho
    Rho     <- reviseRho(data,response=response,fmla=NULL,type=3)
...
# step, if necessary).
    ...
    if(ll < old.ll) {
        decll  <- round(abs(scrit[1]),digits)
        stpnum <- ordinal(em.step)
        whinge <- paste0("\n  At the ",stpnum," step, there was a DECREASE in the\n",
                         "  scaled log likelihood in the amount of ",decll,". The EM\n",
